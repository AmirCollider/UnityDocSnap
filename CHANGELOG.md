...
> **Licence and packaging, September 2026.** Unity DocSnap is no longer MIT
> licensed and no longer ships as source. The package now installs as one
> compiled, Editor-only assembly from the same git URL, and the terms are in
> [LICENSE](LICENSE): Free stays free for any project with no time limit,
> everything the tool generates stays unconditionally yours, and decompiling
> the assembly to satisfy yourself that it is safe to run is explicitly
> allowed. Copies obtained under the MIT licence — everything up to and
> including the `v1.0.3` tag — keep it; that grant was irrevocable.
>
